...
def update_forcings(t):
    ...
    # calculat average bed shear stress field over a given time
    if t == t_s:
        add.assign(shear)
    ...
    if t == t_e:
        # average bed shear
        average_shear = Function(VectorFunctionSpace(mesh2d, 'DG', 1)).interpolate(add / ((t_e - t_s) / Dt))
        File('outputs/average_bed_shear_stress.pvd').write(average_shear)

        print_output("Exporting average_shear")
        average_shear_CG = Function(VectorFunctionSpace(mesh2d, 'DG', 1), name='average_shear').project(average_shear)
        checkpoint_file = checkpointing.DumbCheckpoint(outputdir + '/average_shear_test_' + str(t))
        ...
        velocity_CG = Function(VectorFunctionSpace(mesh2d, 'DG', 1), name='average_velocity').project(average_velocity)
        ...
        File('outputs/bed_shear_max.pvd').write(bed_shear_max)
        print_output("Exporting bed_shear_max")
        bed_shear_max_CG = Function(VectorFunctionSpace(mesh2d, 'DG', 1), name='bed_shear_max').project(bed_shear_max)
