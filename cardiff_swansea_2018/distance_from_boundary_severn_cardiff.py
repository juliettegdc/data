...
# bcs = [DirichletBC(V, 0.0, 1), DirichletBC(V, 2e5, 10)]

bcs = [DirichletBC(V, 0.0, 5), DirichletBC(V, 2e5, 7)] #add a bcs for swansea lagoon too?
v = TestFunction(V)
u = Function(V)
...
bc2 = [DirichletBC(V, 0.0, 14), DirichletBC(V, 0.0, 15),          # Double check that
       DirichletBC(V, 0.0, 16), DirichletBC(V, 0.0, 17),]
...
epss = [50000., 25000., 10000., 5000., 1000., 500., 200., 100., 75.]
for i, eps in enumerate(epss):
  print("Solving Eikonal with eps == ", float(eps))
...
def zbedf(bathy, distance):
    zbed = conditional(le(distance,100),30.0 - distance/100 * 5,
                       conditional(le(distance,300),25,
                                   conditional(le(distance,1000),Max(25-(distance-300)/1000*40,bathy),bathy)))
    return zbed
...
# 3 is the inlet of the basin - I'm letting the distance function there start at 1e5
# Set up hydraulic structures
bc3 = [DirichletBC(V, 0.0, 6), DirichletBC(V, 0.0, 7),          # Double check that
...
epss = [50000., 25000., 10000., 5000., 1000., 500., ]
for i, eps in enumerate(epss):
  print("Solving Eikonal with eps == ", float(eps))
...
bc2 = [DirichletBC(V, 0.0, 1), DirichletBC(V, 0.0, 2),          # Double check that
       DirichletBC(V, 0.0, 3), DirichletBC(V, 0.0, 4), DirichletBC(V, 1e8, 17)] #add swansea ?
v2 = TestFunction(V)
u2 = Function(V)
...
mu_manning = Function(V).interpolate(conditional(le(bath,30),0.023, 0.018))
...
bath_DG = Function(DG, name = "bathymetry").interpolate(bath)

File(outputdir + '/bath.pvd').write(bath_DG)
