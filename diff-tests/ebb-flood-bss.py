...
for i in range(len(tees_flood)):
    a = int((tees_flood[i] + tees3_flood[i] + tees4_flood[i] + tees5_flood[i] + tees6_flood[i]) / (5))
    av_t_flood.append(a)
print(av_t_flood)

av_t_ebb = []
for i in range(len(tees_ebb)):
    a = int((tees_ebb[i] + tees3_ebb[i] + tees4_ebb[i] + tees5_ebb[i] + tees6_ebb[i]) / (5))
    av_t_ebb.append(a)
print(av_t_ebb)
...
checkpoint_file = checkpointing.DumbCheckpoint(file_location+
                                               '/shear_' +str(av_t_ebb[0]), mode=FILE_READ)
...
shear_ebb_data_set = np.empty((len(av_t_ebb), magnitude_ebb.dat.data.shape[0]))
shear_flood_data_set = np.empty((len(av_t_ebb), magnitude_ebb.dat.data.shape[0]))

print (shear_ebb_data_set.shape, shear_flood_data_set.shape)
...
for i in range(len(av_t_ebb)): #at every time of ebb
    ...
    if len(str(av_t_ebb[i]))==2:
        # ebb
        checkpoint_file = checkpointing.DumbCheckpoint(file_location +
                                                       '/shear_' + str(av_t_ebb[i]), mode=FILE_READ)
        checkpoint_file.load(field_2d)
        checkpoint_file.close()
        ex = VelocityMagnitudeSolver(magnitude_ebb, field_2d, 0, min_val=0, solver_parameters={'snes_type': 'newtonls'})
        ex.solve()
        shear_ebb_data_set[i, :] = magnitude_ebb.dat.data[:]

        # flood
        checkpoint_file = checkpointing.DumbCheckpoint(file_location +
                                                       '/shear' + str(av_t_flood[i]), mode=FILE_READ)
        checkpoint_file.load(field_2d)
        checkpoint_file.close()
        ex = VelocityMagnitudeSolver(magnitude_flood, field_2d, 0, min_val=0,
                                     solver_parameters={'snes_type': 'newtonls'})
        ex.solve()
        shear_flood_data_set[i, :] = magnitude_flood.dat.data[:]
...
for i in range(magnitude_ebb.dat.data.shape[0]):
    # ebb
    thetis_shear_ebb = shear_ebb_data_set[:, i]
    thetis_shear_ebb = thetis_shear_ebb.mean()

    mean_ebb.append(thetis_shear_ebb)

    # flood
    thetis_shear_flood = shear_flood_data_set[:, i]
    thetis_shear_flood = thetis_shear_flood.mean()

    mean_flood.append(thetis_shear_flood)

mean_ebb1.dat.data[:] = np.array(mean_ebb)[:]
File('mean_bss_ebb.pvd').write(mean_ebb1)

mean_flood1.dat.data[:] = np.array(mean_flood)[:]
File('mean_bss_flood.pvd').write(mean_flood1)
